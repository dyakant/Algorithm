        ...
        for (int i = 0; i < seats.length; i++) {
            if (seats[i] == 0) { // если 0, то просто считаем их
                ...
                if (left == -1) { // если это первая 1, то
                    dist = Math.max(countZero, dist); // проверяем возможные посчитанные 0 до первой единицы
                } else {
                    countZero = (i - left) / 2; // иначе считаем расстояние между двумя 1
                    dist = Math.max(countZero, dist); // и сравниваем больше ли оно последнего рассчитанного расстояния
                }
